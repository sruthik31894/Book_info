...
@Component({
  ...
  template: `
    ...
    <hr>
    <div class="row banner"><div class="col-12"><h1>{{newBook?.name}}</h1><span *ngIf="authService.getCurrentUser()?.isAdmin" style="float:right !improtant"><a href="/updatebook/{{ newBook?._id }}">Update Book</a> | <button (click)="deleteSingleBook(newBook._id)">DELETE</button></span></div></div>
        ...
        <div *ngIf="newBook" class="card card-primary">
          ...
          <div class="card-block col-12" *ngFor="let review of newBook.reviews">
            ...
            <div  class="col-12 no-gutters review-header card-title">
              ...
              <span *ngIf="authService.isLoggedin() && authService.getCurrentUser().username === review.author" class="review" style=" text-transform: capitalize;"><a href="/book/{{ newBook._id }}/reviews/{{review._id}}">{{review.author}}</a><small class="review">{{review.createdOn | date }}</small></span>
